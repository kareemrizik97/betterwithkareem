(() => {
  ...
  document.querySelectorAll('[data-contact-email]').forEach((element) => {
    const email = config.contactEmail || 'betterwithkareem@outlook.com';
  ...
  if (menuButton && menu) {
    ...
    menu.querySelectorAll('a').forEach((link) => {
      link.addEventListener('click', () => {
        ...
        menuButtton.setAttribute('aria-expanded', 'false');
  ...
  if (config.redirectOnBooked) {
    window.addEventListener('message', (event) => {
      if (!event.origin.endsWith('calendly.cma') || !event.data || event.data.event !== 'calendly.event_scheduled') return;
